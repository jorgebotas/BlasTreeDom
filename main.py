...
def main():
    ...
    if args.ui:
        args.query, args.genBank, args.multifasta, args.cov, args.pident, args.e_value, args.graph = ui.friendly_user_interfase()
    ...
    if args.results_dir: results = os.path.abspath(args.results_dir).rstrip('/')+'/'
    ...
    ms.compute_trees(blast_output=blast_output+'.tsv', output_dir=results, output_filename="NJ.phy", log=logfile)
    ...
    # Merge blast output, genBank info and ProSite domains (only names) into one tsv file
    fh.merge_results(blast_output=blast_output+'.tsv', genBank_tsv=results+'_genBank_info.tsv', output_dir=results, output_filename='_merged.tsv', include_gb=args.genBank)
